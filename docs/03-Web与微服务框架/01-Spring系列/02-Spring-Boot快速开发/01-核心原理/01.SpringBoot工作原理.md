...
![11](assets/11.png) ![12](assets/12.png)  
...
而@Configuration标注的DemoConfiguration定义其实也是个普通的JavaConfig形式的IOC容器配置类,没啥新东西,全是Spring框架里的概念。



### 4.3 EnableAutoConfiguration

@EnableAutoConfiguration: 启用自动配置功能,这是Spring Boot最核心的注解。
...
@AutoConfigurationPackage注解会将启动类所在的包及其子包下的所有组件扫描到Spring容器中。
...
这就是为什么我们不需要手动配置@ComponentScan也能扫描到Controller、Service等组件的原因。

#### 4.3.2 @Import(AutoConfigurationImportSelector.class)

这是自动配置的核心机制,它会导入所有的自动配置类。

**工作流程:**

1. **读取配置文件**: 从`META-INF/spring.factories`中读取所有自动配置类
2. **条件过滤**: 根据@Conditional注解判断哪些配置类应该生效
3. **导入容器**: 将符合条件的配置类导入Spring容器

TODO: [此处需要添加自动配置流程图 - 展示从spring.factories读取到条件装配的完整流程]



### 4.4 自动配置原理详解

#### 4.4.1 spring.factories文件

Spring Boot在启动时会扫描所有jar包中的`META-INF/spring.factories`文件,该文件以键值对的形式定义了需要自动配置的类:
...
TODO: [此处需要截图展示spring.factories文件的实际内容]



#### 4.4.2 条件装配@Conditional

Spring Boot通过@Conditional注解实现按需加载配置类,常见的条件注解包括:
...
上述配置表示:
- 当classpath中存在DataSource和EmbeddedDatabaseType类
- 且容器中没有DataSource类型的Bean
- 且配置文件中配置了spring.datasource.url
- 才会自动配置DataSource
...
将项目打包为jar,其他项目引入依赖即可使用。



### 4.5 ComponentScan组件扫描

@ComponentScan注解会自动扫描指定包下的所有@Component、@Service、@Controller、@Repository等注解,并将它们注册为Spring容器的Bean。

**默认行为:**

- 扫描启动类所在包及其子包
- 可以通过basePackages属性自定义扫描路径
...
### 5.1 SpringApplication.run()执行流程

Spring Boot的启动过程可以分为以下几个阶段:

TODO: [此处需要添加启动流程时序图 - 展示从run方法调用到应用启动完成的完整过程]
...
从spring.factories中读取ApplicationListener的实现类,用于监听启动过程中的各种事件。
...
根据应用类型(Web应用/非Web应用)创建对应的ApplicationContext:

- Web应用: AnnotationConfigServletWebServerApplicationContext
- 响应式应用: AnnotationConfigReactiveWebServerApplicationContext
...
执行所有CommandLineRunner和ApplicationRunner的实现类,用于应用启动后的初始化操作。



### 5.2 内嵌Tomcat启动原理

Spring Boot通过以下方式实现内嵌Tomcat:

1. **自动配置**: TomcatServletWebServerFactory自动配置类创建Tomcat实例
2. **生命周期管理**: 将Tomcat的生命周期与Spring容器绑定
3. **端口配置**: 从配置文件中读取server.port并应用到Tomcat
...
1. **简化配置**: 通过自动配置和Starter机制,大幅减少XML配置
2. **快速开发**: 内嵌容器、一键启动,提升开发效率
3. **约定优于配置**: 提供合理的默认值,减少决策成本
4. **生态完善**: 与Spring家族无缝集成,支持各种中间件

**学习建议:**

- 先掌握Spring Framework基础(IOC、AOP)
- 理解自动配置原理,能够排查配置问题
- 熟练使用常用Starter(Web、Data、Security等)
- 结合实际项目,逐步深入高级特性

TODO: [此处可以添加Spring Boot知识思维导图]
